...
The `fwapgr` package is used to retrieve data from the BC Freshwater Atlas. In this article, we'll get stream, lake, watershed and modelled fish habitat data to create a map of the Yakoun River watershed in Haida Gwaii, BC. 
...
Let's use the 'fwa_stream_networks_sp' layer to get a linestring feature of the Yakoun river. We can filter by 'gnis_name' column and use `fwa_search_gnis_streams` to find a valid gnis_name. 
```{r}
print(fwapgr::fwa_search_gnis_streams("yakoun"))
```

`fwa_feature` is used to retrieve data for any layer. We use the `columns` argument to get the `blue_line_key` column along with the geometry. 
```{r}
...
Next let's get the Yakoun River watershed with `fwa_watershd`, which requires a `blue_line_key`.
```{r}
...
Finally, let's get modelled fish habitat within our watershed bounds. Note, this is in a different schema (not the default 'whse_basemapping' schema). We'll re-classify the 'fish_habitat' column as 'fish habitat' or 'not fish habitat'. 
```{r}
...
To make the map look a little prettier, we can clip the habitat and lake layers to the watershed polygon.
