...
Let's get a linestring feature of the Yakoun river. We can use `fwa_search_gnis_streams` to find a valid gnis_name. 
```{r}
print(fwapgr::fwa_search_gnis_streams("yakoun"))
```

`fwa_gnis_streams` is a convenience function for filtering streams by `gnis_name`. It is the equivalent of `fwa_feature('fwa_stream_networks_sp', filter = 'gnis_name = {gnis_name}')`. We use the `columns` argument to get the `blue_line_key` column along with the geometry. 
```{r}
